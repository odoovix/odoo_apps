...
class Home(WebHome):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        orientation = conf_param.get_param('login_page_custom.orientation')
        image = conf_param.get_param('login_page_custom.image')
        url = conf_param.get_param('login_page_custom.url')
        background_type = conf_param.get_param('login_page_custom.background')
        if background_type == 'color':
            values['bg'] = ''
            values['color'] = conf_param.sudo().get_param(
                'login_page_custom.color')
        ...
        if orientation == 'right':
            response = request.render('login_page_custom.login_template_right',
                                      values)
        elif orientation == 'left':
            response = request.render('login_page_custom.login_template_left',
                                      values)
        elif orientation == 'middle':
            response = request.render('login_page_custom.login_template_middle',
                                      values)
        else:
