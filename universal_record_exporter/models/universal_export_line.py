...
class UniversalExportLine(models.Model):
    """
    One row = one column in the export output, sourced from the *primary* model.
    """
    _name = 'universal.export.line'
    _description = 'Universal Export Main Field'
    _order = 'sequence, id'
    ...
    def _check_field_configuration(self):
        for line in self:
            rule_model = line.export_id.model_id
            if rule_model and line.field_id and line.field_id.model_id != rule_model:
                raise ValidationError(_(
                    'The field "%(field)s" must belong to the export rule model "%(model)s".',
                    field=line.field_id.name,
                    model=rule_model.model,
                ))
