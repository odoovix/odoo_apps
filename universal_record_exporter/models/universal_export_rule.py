...
class UniversalExportRule(models.Model):
    ...
    def _export_excel(self, records):
        try:
            ...
            raise UserError(_(
                'The Python library "xlsxwriter" is required for Excel export.'
            ))
        if not xlsxwriter:
            raise UserError(_('The Python library "xlsxwriter" is required for Excel export.'))

        headers, rows = self._build_rows(records)

        output = io.BytesIO()
        workbook = xlsxwriter.Workbook(output, {'in_memory': True})
        worksheet = workbook.add_worksheet(self._get_sheet_title())
        ...
        widths = [len(str(header or '')) for header in headers]
        for col_idx, header in enumerate(headers):
            worksheet.write(0, col_idx, header, header_format)

        for row_idx, row in enumerate(rows, 1):
            row_format = alt_data_format if row_idx % 2 else data_format
            for col_idx, value in enumerate(row):
                worksheet.write(row_idx, col_idx, value, row_format)
                widths[col_idx] = max(widths[col_idx], len(str(value or '')))

        for col_idx, width in enumerate(widths):
            worksheet.set_column(col_idx, col_idx, min(max(width + 3, 12), 60))
