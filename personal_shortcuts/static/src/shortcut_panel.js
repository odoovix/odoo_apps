...
export class ShortcutPanel extends Component {
    ...
    getMenuIdFromUrl(url) {
        ...
        if (!actionId) {
            const actionPath = this.getOdooActionPath(url);
            if (actionPath) {
                const menu = this.menus
                    .getAll()
                    .find((candidate) => candidate.actionPath === actionPath);
    ...
    getOdooActionPath(url) {
        try {
            const target = new URL(url || "", window.location.origin);
            const marker = "/odoo/";
            if (!target.pathname.includes(marker)) {
                return false;
            }
            const actionPath = target.pathname.split(marker, 2)[1].replace(/^\/+|\/+$/g, "");
            return actionPath.split("/", 1)[0] || false;
        } catch {
            return false;
        }
    }
